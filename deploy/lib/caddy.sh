...
configure_panel_caddy() {
    ...
    touch /var/log/caddy/access_lacmp-panel.log
    chown "${WEB_USER}:${WEB_USER}" /var/log/caddy/access_lacmp-panel.log
    chmod 0640 /var/log/caddy/access_lacmp-panel.log

    install -d -m 0755 "${CADDY_CONFD}"
    local snippet="${CADDY_CONFD}/lacmp-panel.conf"
