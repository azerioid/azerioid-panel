    ...
    case "${PKG_MGR}" in
        apt-get)
            ...
            apt-get -o DPkg::Lock::Timeout=120 install -y \
                caddy sqlite3 curl ca-certificates gnupg \
                "php${PANEL_PHP_VERSION}-fpm" "php${PANEL_PHP_VERSION}-cli" \
                "php${PANEL_PHP_VERSION}-sqlite3" "php${PANEL_PHP_VERSION}-mbstring" \
                "php${PANEL_PHP_VERSION}-xml" "php${PANEL_PHP_VERSION}-curl" \
                "php${PANEL_PHP_VERSION}-zip" "php${PANEL_PHP_VERSION}-bcmath" \
                unzip git
            ;;
        dnf)
            dnf -y install caddy sqlite curl ca-certificates gnupg2 unzip git \
                php-fpm php-cli php-sqlite3 php-mbstring php-xml php-curl \
